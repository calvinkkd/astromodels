...
file_names = [
    ...
    for path, _, filenames in os.walk("lib")
...
large_files = filter(
    lambda x: x[1] > 1.0 and os.path.splitext(x[0])[1] == ".fits", file_sizes
)
...
subprocess.check_call(
    "tar c lib info | bzip2 > ../%s/%s-%s-0.tar.bz2"
    % (system, index["name"], index["version"]),
    shell=True,
)
