...
class Asymm_Gaussian_on_sphere(Function2D):
    ...
    def evaluate(self, x, y, lon0, lat0, a, e, theta):
        ...
        idx = np.logical_and(
            np.logical_or(dlon < 0, dlon > 180), np.logical_or(dlon > -180, dlon < -360)
        )
        dX[idx] = -dX[idx]
        ...
        return np.power(180.0 / np.pi, 2) * 1.0 / (2 * np.pi * a * b) * np.exp(E)
...
class Disk_on_sphere(Function2D):
    ...
    def evaluate(self, x, y, lon0, lat0, radius):
        ...
        return (
            np.power(180 / np.pi, 2) * 1.0 / (np.pi * radius ** 2) * (angsep <= radius)
        )
...
class Power_law_on_sphere(Function2D):
    ...
    def get_boundaries(self):
        ...
        return (
            ((self.lon0.value - self.maxr.value), (self.lon0.value + self.maxr.value)),
