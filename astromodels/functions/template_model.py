    ...
    def add_interpolation_data(self, differential_fluxes, **parameters_values_input):
        ...
        if self._data_frame is None:
            ...
            self._multi_index = pd.MultiIndex.from_product(
                self._parameters_grids.values(), names=self._parameters_grids.keys()
            )
...
class TemplateModel(Function1D):
    ...
    def _prepare_interpolators(self, log_interp):
        ...
        for energy in self._energies:
            ...
            if len(self._parameters_grids.values()) == 2:
                ...
                this_interpolator = scipy.interpolate.RegularGridInterpolator(
                    self._parameters_grids.values(), this_data
                )
    ...
    def _interpolate(self, energies, scale, parameters_values):
        ...
        log_interpolations = np.array(
            map(
                lambda i: self._interpolators[i](np.atleast_1d(parameters_values)),
        ...
        if self._is_log10:
            ...
            interpolator = scipy.interpolate.InterpolatedUnivariateSpline(
                e_tilde, log_interpolations, k=self._interpolation_degree, ext=0
            )
