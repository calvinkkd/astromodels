...
from astromodels.sources.source import (
    Source,
    POINT_SOURCE,
    EXTENDED_SOURCE,
    PARTICLE_SOURCE,
...
class Model(Node):
    ...
    def _repr__base(self, rich_output=False):
        ...
        sources_summary = pd.DataFrame.from_items(
            (
        ...
        if self._independent_variables:
            ...
            for (
                variable_name,
                variable_instance,
            ) in self._independent_variables.iteritems():
    ...
    def get_extended_source_boundaries(self, id):
        ...
        (ra_min, ra_max), (dec_min, dec_max) = self._extended_sources.values()[
            id
        ].get_boundaries()
